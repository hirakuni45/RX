...
Arduino はＣ＋＋ですが、フレームワークの仕様をある程度制限する事で、Ｃ言語ユーザーの多くに受け入れられています。   
それからすると、Arduino が扱える人は、Ｃ＋＋でも大きなハードルにはならないと思えます。   
   
Ｃ＋＋は独学で学ぶには限界があり、勉強会（非常に盛んに開かれています）などで、より多くを学んだ先人に Tips を学ぶ事で大きな成果を得られます。   
...
OPTIMIZE    =   -O3
```
上記はｍ最大の最適化です、場合により、「-Os」（サイズ優先）、「-O2」、「-O1」など使えます。   
※現在、Ｃ＋＋フレームワークのテンプレートリソースの問題で、「-O0」（最適化無し）が正常にコンパイル出来ません。   
   
### ビルド、サブディレクトリーの変更
   
ビルド時、オブジェクトの格納場所を以下のキーワードで制御します。   
```
BUILD       =   release
```
※上記では、「release」ディレクトリが使われる。   
