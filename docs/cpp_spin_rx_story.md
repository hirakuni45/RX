...
これは、ＲＸマイコンが良さそうで評価している最中、Ｃ＋＋の学習を兼ねて、「より良い」ＲＸマイコン用フレームワークを実装しようと思い、始めました。   
   
それから、地道に、色々な実装をして、改良を続け、最近ようやく、実用レベルの段階に来たと思えます。   
※実際にいくつかの仕事で利用しており、製品として使っています。   
   
このフレームワークは、Ｒ８Ｃ、ＲＬ７８、ＲＸマイコン用に実装しているもので、全てＣ＋＋で書かれています。   
※一部、Ｃのソースがある。   
   
特徴として：   
- マイコン固有のデバイスＩ／Ｏ定義を、Ｃ＋＋のテンプレートクラスライブラリで実装している。   
- 基本、gcc、g++、make だけあれば使う事ができる。   
- 各種デバイスを簡単に使う為、デバイスドライバーをＣ＋＋により、独自に実装している。   
  ※もちろん、ルネサス社が公開しているソースコードを利用する事も出来る。   
- git にソースコードの全てを公開している。   
- ＭＩＴライセンスで公開している。   
- Ｉ２ＣやＳＰＩ接続の色々なデバイスドライバーを公開している。   
- 今も改良を続けており、進化している。   
- 豊富なサンプルプログラムも公開している。   
...
また、MSYS2 のインストールを複数のマシンで行う場合等を考えると、ホームとは別のディレクトリー上で行う方が利便性が良いと多いと感じています。   
※これは Windows 環境に限った話です。   
...
最近は、ＲＸマイコンの開発でも「boost」を使う事が標準的になっています。   
...
初期段階で、サンプルプログラムが書き込まれていて、ＲＸ６５Ｎのパフォーマンスを確認出来ますが、一通り、遊んだら、早速、自分で作ったプログラムをロードして動かしてみましょう。   
