...
ＰＣ上や携帯電話で動くアプリケーションと同様に、組み込みマイコンでもプログラミングを行う作業は必須ですが、ＰＣ上のアプリケーションでは、プログラムを作成すれば、直に動作を確認できます。   
...
やり方は人それぞれで、その人の能力や性格にもよると思いますが、まずは自分の考え方から紹介してみようと思います。   
...
※これはＣ言語でもそうですが、異なった環境に対する対応をスマートに表現出来ない事が多いです。   
...
ステップ実行出来ない事が余り不利益にならない。   

ソースレベルデバッガーと J-TAG（ハードレベルデバッガー的なもの）の組み合わせは、非常に強力で柔軟な開発環境を提供してきました。   
一旦この環境が浸透してしまうと、精神的にも、肉体的（物理的）にもこの環境から抜け出せない開発者が多く発生してしまい、応用したり工夫する力、想像や推定など、基本的な能力が衰退してしまいました。   

J-TAG 技術は、定められた手順で、マイコン内部の状態を物理的にスキャンする事が可能です。   
ほとんどの技術者は、J-TAG 等による開発環境が無い現場を敬遠して、これが無いと「まともな開発」は出来ないものと認識している人が多数いる現実があります。   
...
他に、もっと単純なのは、余っているポートを出力に設定して、文字出力の代わりに、何らかの信号を出力して、ロジックアナライザやテスターなどで確認する事も出来ます。   
...
ただ、C++ の習得にはそれなりに長い時間がかかる事を覚悟する必要があります。   
C 言語がある程度扱えるのであれば、少しづつ、C++ の守備範囲を広げていけば良い訳で、あまり神経質になる必要はありません。   
Arduino は C++ ですが、フレームワークの仕様をある程度制限する事で、C 言語ユーザーの多くに受け入れられています。   

C++ は独学で学ぶには限界があり、勉強会などで、より多くを学んだ先人に Tips を学ぶ事には利益が大きいと思います。  
...
---
 ...
 - ビルドしたコンパイラーコレクションを以下のリンクに上げてある。（MSYS2 のみ）   
...
<img src="RTK5RX65N.jpg" width="30%">
...
ルネサス社は、RX65N の販促目的でコストパフォーマンスに優れたガジェットとして、RX65N Envision Kit を発売しました。   
 ...
 - 12bits D/A 出力２チャンネル
 - 12bits A/D 入力複数チャネル
 - 簡易デバッグ環境を標準搭載（Mac、Linux 環境では利用出来ない）   
 - 100bps イーサーネット   
...
色々、要望もありますが、入手しやすく、それなりに安価（￥５４００）で、高機能なので、初めてＲＸマイコンを始めるには良い機器と言えます。   
...
※ 残念ですが、Mac や Linux では、内臓 J-TAG 機能を利用出来ないので、Windows が必要です。   
   
初期段階で、サンプルプログラムが書き込まれていて、ＲＸ６５Ｎのパフォーマンスを確認出来ますが、一通り、遊んだら、早速、自分で作ったプログラムを書き込んでみましょう。   
...
そこで、秋月電子で販売している「マイクロＳＤカードスロットＤＩＰ化キット」を取り付ける事が簡単そうです。   
   
 - オーディオ出力
   
ＲＸ６５Ｎには、１２ビットのＤ／Ａ出力が２チャンネルあり、オーディオ出力として使えます。   
ただ、標準では、ＳＷ２、ＳＷ３に接続されています。   
※ＳＷ３は未実装   
オペアンプを追加して、

### プログラムの書き込み方法
   
ＳＷ１の１を「ＯＮ」にする事で、デバッグモードになり「Renesas Flash Programmer」を使って、プログラムを書き込む事が出来ます。   
ＳＷ１の１を「ＯＦＦ」にすると通常モードで、内臓プログラムが走ります。   
...
注意していますが、中途な状態で、ソースコードをプッシュして、コンパイル出来ない場合などあると思いますが、その場合、メールや twitter などで連絡して下さい。   
