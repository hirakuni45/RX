...
#include "../rx64m_sed/core.hpp"
#include "../rx64m_sed/tools.hpp"
#include "../rx64m_sed/nets.hpp"
...
namespace {
	...
	{
		nets_.at_net_main().at_ethernet().at_ethd().enable_interrupt(false);
		while(putch_tmp_.length() > 0) {
			core_.sci_.putch(putch_tmp_.get());
		}
		nets_.at_net_main().at_ethernet().at_ethd().enable_interrupt();
	}
}
