...
namespace seeda {
	...
	static const int seeda_version_ = 300;
...
#ifdef SEEDA
	typedef chip::phy_base<ETHERC, 0x01, chip::phy_option::TI_DP83822> PHY;  // Ethernet PHY (TI/DP83822)
#else
	typedef chip::phy_base<ETHERC> PHY;  // Ethernet PHY (LAN8720)
#endif
	typedef device::ether_io<ETHERC, EDMAC, PHY> ETHD;  // EthernetDriver

	static const uint32_t UDPN = 2;  // UDP の経路数
 	static const uint32_t TCPN = 6;  // TCP の経路数
	typedef net::net_main<ETHD, UDPN, TCPN> NET_MAIN;
	static const uint32_t HTTP_MAX_LINK = 16;
	static const uint32_t HTTP_MAX_BUFF = 8192;
	typedef net::http_server<NET_MAIN::ETHERNET, SDC, HTTP_MAX_LINK, HTTP_MAX_BUFF> HTTP;
