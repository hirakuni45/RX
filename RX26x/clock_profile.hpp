...
namespace device {
	...
	/*!
		@brief  クロック・プロファイル・クラス @n
				...
				・HOCO を使う場合、OFS1 レジスタがデフォルトだと 32MHz となりそれが最大速度となる @n
				・詳細はハードウェアーマニュアル参照の事
	*/
	//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
	class clock_profile {
	public:
		...
		/*!
			@brief  発信器タイプ @n
					...
					HOCO を使う場合、BASE と PLL_BASE を同じ周波数に設定する @n
		...
		*/
		...
		static constexpr uint32_t   BASE		=  8'000'000;		///< 外部接続クリスタル（1MHz ～ 24MHz）
		...
		static constexpr uint32_t   PLL_BASE	= 32'000'000;		///< PLL ベースクロック（最大 24 to 64MHz）

		static constexpr uint32_t   ICLK		= 32'000'000;		///< ICLK 周波数（最大 64MHz）
		static constexpr uint32_t   PCLKA		= 32'000'000;		///< PCLKA 周波数（最大 64MHz）
		static constexpr uint32_t   PCLKB		= 32'000'000;		///< PCLKB 周波数（最大 32MHz）
		static constexpr uint32_t   PCLKD		= 32'000'000;		///< PCLKD 周波数（最大 64MHz）
		static constexpr uint32_t   FCLK		= 32'000'000;		///< FCLK 周波数（最大 64MHz）
		static constexpr uint32_t   CANFDCLK	= 32'000'000;		///< CANFD 周波数（最大 32MHz）
#endif
		// RXv3 ノーウェイトの場合の調整値
