	...
	*/
	...
	class port_map_mtu : public port_map_order {
		...
		static bool mtu4_(CHANNEL ch, bool ena, ORDER odr) noexcept
		{
			...
			// PE5 (LFQFP100:  73) (LFQFP144: 106) (LFQFP176: 130)
				...
				case ORDER::THIRD:
					PORT8::PMR.B7 = 0;
					MPC::P87PFS.PSEL = ena ? 0b00'1000 : 0;  // ok
					PORT8::PMR.B7 = ena;
					break;
				case ORDER::FOURTH:
					PORTB::PMR.B1 = 0;
			...
			// PE4 (LFQFP100:  74) (LFQFP144: 107) (LFQFP176: 131)
				...
				case ORDER::THIRD:
					PORT8::PMR.B6 = 0;
					MPC::P86PFS.PSEL = ena ? 0b00'1000 : 0;  // ok
					PORT8::PMR.B6 = ena;
					break;
				case ORDER::FOURTH:
					PORTC::PMR.B3 = 0;
		...
		{
			bool ret = true;
			uint8_t sel = ena ? 0b00'1000 : 0;  // ok
			switch(ch) {
	...
	public:
		...
		/*!
			...
			@param[in]	neg	反転入出力の場合「true」（RX64M/RX71M では未サポート）
